...
async function parseWorkflowFile(path) {
  ...
  try {
    const workflow = (0, import_yaml2.parse)(content);
    return workflow;
  } catch (error) {
...
program2.name("gh-actions-lockfile").description(
...
var outputOption = ["-o, --output <path>", "Path to lockfile", DEFAULT_PATH];
var tokenOption = [
  "-t, --token <token>",
  "GitHub token (or use GITHUB_TOKEN env var)"
];
program2.command("generate").description("Generate or update the lockfile").option(...workflowsOption).option(...outputOption).option(...tokenOption).action(async (options) => {
  try {
...
program2.command("verify").description("Verify workflows match the lockfile").option(...workflowsOption).option(...outputOption).option("-c, --comment", "Post PR comment on verification failure", true).action(async (options) => {
  try {
...
program2.command("list").description("Display the dependency tree of all locked actions").option(...workflowsOption).option(...outputOption).action(async (options) => {
  try {
    await list(options);
  } catch (error) {
    console.error("Error:", error instanceof Error ? error.message : error);
